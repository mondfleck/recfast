module recfast_wrapper
    ...
    contains
        ...
        subroutine recfast_c(Omega_b, Omega_c, Omega_L, H0, T_CMB, Yp, H_switch, He_switch, &
                             z_initial, z_final, tol, Nz, z_array, x_array, xH_array, xHe_array, Tmat_array) bind(c)
        ...
        real(c_double), intent(out) :: xH_array(Nz)
        real(c_double), intent(out) :: xHe_array(Nz)
        real(c_double), intent(out) :: Tmat_array(Nz)
        call recfast_func(Omega_b, Omega_c, Omega_L, H0, T_CMB, Yp, H_switch, He_switch, &
                          z_initial, z_final, tol, Nz, z_array, x_array, xH_array, xHe_array, Tmat_array)
